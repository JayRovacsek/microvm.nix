{ nixpkgs-lib }: rec {
  ...
  withDriveLetters = { volumes, hypervisor, storeOnDisk, ... }:
    let
      offset = (if hypervisor == "cloud-hypervisor"
      # requires bootDisk
      then
        1
      else
        0) + (if storeOnDisk then 1 else 0);
    in map ({ fst, snd, }: fst // { letter = snd; })
    (nixpkgs-lib.zipLists volumes
      (nixpkgs-lib.drop offset nixpkgs-lib.strings.lowerChars));

  createVolumesScript = pkgs:
    pkgs.lib.concatMapStringsSep "\n" ({ image, size ?
        throw "Specify a size for volume ${image} or use autoCreate = false"
      , fsType ? defaultFsType, autoCreate ? true, ... }:
      nixpkgs-lib.optionalString autoCreate ''
        PATH=$PATH:${
          with pkgs;
          lib.makeBinPath [ coreutils util-linux e2fsprogs ]
        }
